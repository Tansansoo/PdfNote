...
## 현재 단계: 3단계 — S펜 필기
- 홈 화면(보관함): 가져온 PDF 목록(썸네일·제목·쪽수). "PDF 가져오기"는 원본을 건드리지 않고 앱 안에 복사본을 저장. 길게 눌러 이름 변경/삭제
- 1단계: PDF 열기, 세로 스크롤, 두 손가락 핀치 줌 (1~5배), 페이지 번호
- 화면 분할: 왼쪽 PDF / 오른쪽 워크스페이스 (가운데 손잡이로 비율 조절, 좁은 화면은 위/아래)
- 워크스페이스: 무한 캔버스 (한 손가락 이동, 두 손가락 줌), 카드 드래그 이동, 탭 선택, ✕로 삭제, 오른쪽 아래 손잡이로 크기 조절
...
- 필기: 펜/형광펜/지우개(획·영역), 직선 모드, 그리다 멈추면 직선으로, S펜 측면 버튼 = 지우개, 실행취소/다시실행. 색은 표준 팔레트/사용자 지정(색상환, HEX)에서 고르고 팔레트에 저장 가능. PDF 페이지와 워크스페이스 모두 가능, 문서별 자동 저장

다음 단계: 글자 단위 텍스트 선택/발췌 → 카드 정리 기능
